...
map({MODKEY, mod.SHIFT}, "f", function()
    local c = cwc.client.focused()
    if c then c.fullscreen = not c.fullscreen end
end, { description = "toggle fullscreen", group = "client" })

map(MODKEY, "f", function()
    local c = cwc.client.focused()
