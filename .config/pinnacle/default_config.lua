...
Pinnacle.setup(function()
    ...
    if Pinnacle.backend() == 'window' then mod_key = 'alt' end
    ...
    Input.mousebind({ mod_key }, 'btn_left', function() Window.begin_move 'btn_left' end, {
        group = 'Mouse',
    ...
    Input.mousebind(
        { mod_key },
        'btn_right',
        function() Window.begin_resize 'btn_right' end,
        { group = 'Mouse', description = 'Start an interactive window resize' }
    )
    ...
    if Snowcap then
        Input.keybind({ mod_key }, 's', function() Snowcap.integration.bind_overlay():show() end, {
            group = 'Compositor',
    ...
    if Snowcap then
        ...
        Input.keybind {
            mods = { mod_key, 'shift' },
            key = 'q',
            on_press = function() Snowcap.integration.quit_prompt():show() end,
            group = 'Compositor',
    ...
    Input.keybind({ mod_key }, key.Return, function() Process.spawn(terminal) end, {
        group = 'Process',
        description = 'Spawn a terminal',
    })

    Input.keybind({ mod_key }, 'd', function() Process.spawn 'fuzzel' end, {
        group = 'Process',
        description = 'Spawn a terminal',
    })

    -- mod_key + ctrl + space = Toggle floating
    Input.keybind({ mod_key }, 'g', function()
        local focused = Window.get_focused()
        if focused then
    ...
    Input.keybind({ mod_key }, key.space, function()
        ...
        if focused_op then
            ...
            for i, t in ipairs(tags) do
                tag_actives[i] = function() return t:active() end
            end
    ...
    for _, tag_name in ipairs(tag_names) do
        ...
        Input.keybind({ mod_key }, tag_name, function() Tag.get(tag_name):switch_to() end, {
            group = 'Tag',
            description = 'Switch to tag ' .. tag_name,
        })

        -- mod_key + ctrl + 1-9 = Toggle tags 1-9
        Input.keybind({ mod_key, 'ctrl' }, tag_name, function() Tag.get(tag_name):toggle_active() end, {
            group = 'Tag',
    ...
    Libinput.for_each_device(function(device)
        ...
        if device:device_type() == 'touchpad' then device:set_natural_scroll(true) end
    end)
    ...
    Window.add_window_rule(function(window) window:set_decoration_mode 'client_side' end)

    -- Enable sloppy focus
    Window.connect_signal {
        pointer_enter = function(window) window:set_focused(true) end,
    }

    -- Spawning should happen after you add tags, as Pinnacle currently doesn't render windows without tags.
    Process.spawn_once(terminal)
    Process.spawn_once 'waybar'
end)
